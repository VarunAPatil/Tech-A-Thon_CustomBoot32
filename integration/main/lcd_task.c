...
void ui_update_values(int temp_x100, int units, const char *lat, const char *ns, const char *lon, const char *ew, int sats)
{
    ...
    /* GPS Coordinates */
    char latbuf[32];
    snprintf(latbuf, sizeof(latbuf), "LAT: %s %s", lat, ns);
    lv_label_set_text(label_gps_lat, latbuf);
...
#define COL_BG_DEEP    lv_color_hex(0x060B18)
#define COL_BG_CARD    lv_color_hex(0x0D1B2E)
#define COL_BLUE_BRITE lv_color_hex(0x4D9FFF)
#define COL_BLUE_ICE   lv_color_hex(0xA8D4FF)
#define COL_BLUE_DIM   lv_color_hex(0x1B3A5C)
#define COL_TEXT_HI    lv_color_hex(0xE6F0FF)
#define COL_TEXT_MID   lv_color_hex(0x7BA8D4)
#define COL_TEXT_LO    lv_color_hex(0x3A5A80)
...
static void ui_create(void)
{
    ...
    lv_obj_set_size(card_temp, 220, 65);
    ...
    lv_obj_align(lbl_tname, LV_ALIGN_TOP_LEFT, 16, 5);
    ...
    lv_obj_align(row_temp, LV_ALIGN_LEFT_MID, 16, 10);
    ...
    lv_obj_align(bar_temp, LV_ALIGN_BOTTOM_LEFT, 16, -5);
    ...
    lv_obj_set_size(card_units, 220, 65);
    lv_obj_align(card_units, LV_ALIGN_TOP_MID, 0, 126);
    ...
    lv_obj_align(lbl_uname, LV_ALIGN_TOP_LEFT, 16, 5);
    ...
    lv_obj_set_flex_align(row_units,
        ...
        LV_FLEX_ALIGN_CENTER,
        LV_FLEX_ALIGN_CENTER);
    lv_obj_align(row_units, LV_ALIGN_LEFT_MID, 16, 10);
     ...
     *  GPS CARD
     * ══════════════════════════════════════════ */
    /* Wait, the screen is only 320px tall. Let's make the cards smaller to fit. */
    lv_obj_t *card_gps = lv_obj_create(scr);
    lv_obj_set_size(card_gps, 220, 80);
    lv_obj_align(card_gps, LV_ALIGN_TOP_MID, 0, 200);
    lv_obj_set_style_bg_color(card_gps, COL_BG_CARD, 0);
    lv_obj_set_style_bg_opa(card_gps, LV_OPA_COVER, 0);
    lv_obj_set_style_border_color(card_gps, COL_BLUE_DIM, 0);
    lv_obj_set_style_border_width(card_gps, 1, 0);
    lv_obj_set_style_radius(card_gps, 6, 0);
    lv_obj_set_style_pad_all(card_gps, 0, 0);

    /* Left strip */
    lv_obj_t *strip3 = lv_obj_create(card_gps);
    lv_obj_set_size(strip3, 4, 64);
    lv_obj_align(strip3, LV_ALIGN_LEFT_MID, 0, 0);
    lv_obj_set_style_bg_color(strip3, COL_BLUE_BRITE, 0);
    lv_obj_set_style_bg_opa(strip3, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(strip3, 0, 0);
    lv_obj_set_style_radius(strip3, 0, 0);

    /* "GPS" label */
    lv_obj_t *lbl_gname = lv_label_create(card_gps);
    lv_label_set_text(lbl_gname, "GPS LOC");
    lv_obj_set_style_text_color(lbl_gname, COL_TEXT_MID, 0);
    lv_obj_set_style_text_font(lbl_gname, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_letter_space(lbl_gname, 2, 0);
    lv_obj_align(lbl_gname, LV_ALIGN_TOP_LEFT, 16, 5);

    /* Values */
    label_gps_lat = lv_label_create(card_gps);
    lv_label_set_text(label_gps_lat, "LAT: ----");
    lv_obj_set_style_text_color(label_gps_lat, COL_BLUE_ICE, 0);
    lv_obj_set_style_text_font(label_gps_lat, &lv_font_montserrat_14, 0);
    lv_obj_align(label_gps_lat, LV_ALIGN_TOP_LEFT, 16, 25);

    label_gps_lon = lv_label_create(card_gps);
    lv_label_set_text(label_gps_lon, "LON: ----");
    lv_obj_set_style_text_color(label_gps_lon, COL_BLUE_ICE, 0);
...
void lcd_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (xSemaphoreTake(sensor_data_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            ...
            strcpy(current_gps_ew, shared_gps_ew);
